        ...
        loop {
            ...
            match ext_type {
                HEADER_EXT_BACKING_FORMAT => {
                    ...
                    header.backing_file_format = Some(format_str.parse()?);
                }
                _ => {
...
impl BackingFile {
    fn new(
        ...
        backing_file_format: Option<ImageType>,
        direct_io: bool,
...
impl QcowFile {
    ...
    pub fn new_from_backing(
        ...
        header.backing_file_format = Some(backing_format.parse()?);
        QcowFile::new_from_header(file, &header)
...
mod unit_tests {
    ...
    fn read_header_extensions_unknown_extension() {
        ...
        assert_eq!(header.backing_file_format, None);
    }
    ...
    fn read_header_extensions_raw_format() {
        ...
        QcowHeader::read_header_extensions(&mut disk_file, &mut header).unwrap();
        assert_eq!(header.backing_file_format, Some(ImageType::Raw));
    }
    ...
    fn read_header_extensions_qcow2_format() {
        ...
        QcowHeader::read_header_extensions(&mut disk_file, &mut header).unwrap();
        assert_eq!(header.backing_file_format, Some(ImageType::Qcow2));
    }
