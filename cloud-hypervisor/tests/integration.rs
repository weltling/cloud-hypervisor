...
mod common_parallel {
    ...
    fn test_compute_file_checksum_different_data() {
        let data1 = b"data1";
        let data2 = b"data2";
        let mut reader1 = std::io::Cursor::new(data1);
        let mut reader2 = std::io::Cursor::new(data2);
    ...
    fn compute_backing_checksum(
        ...
        let mut file = std::fs::File::open(&backing_path).ok()?;
    ...
    fn disk_check_consistency(
        ...
        if let Some((backing_path, format, initial_checksum)) = initial_backing_checksum {
            ...
            let mut file = std::fs::File::open(&backing_path).unwrap();
