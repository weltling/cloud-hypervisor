...
pub fn configure_vcpu(
    ...
    if matches!(cpu_vendor, CpuVendor::Intel) {
        ...
        CpuidPatch::set_cpuid_reg(&mut cpuid, 0x8000_001e, Some(0), CpuidReg::EAX, u32::from(id));
    }
    ...
    if matches!(cpu_vendor, CpuVendor::AMD) {
        CpuidPatch::set_cpuid_reg(&mut cpuid, 0x8000_0001, Some(0), CpuidReg::ECX, (1u32 << 1) | (1u32 << 22));
    }
...
fn update_cpuid_topology(
    ...
    if matches!(cpu_vendor, CpuVendor::Intel) {
        ...
        CpuidPatch::set_cpuid_reg(cpuid, 0x8000_001e, Some(0), CpuidReg::EBX, ((threads_per_core as u32 - 1) << 8) | (id as u32 & 0xff));
        CpuidPatch::set_cpuid_reg(cpuid, 0x8000_001e, Some(0), CpuidReg::ECX, ((dies_per_package as u32 - 1) << 8) | (thread_width + die_width) & 0xff);
        CpuidPatch::set_cpuid_reg(cpuid, 0x8000_001e, Some(0), CpuidReg::EDX, 0);
        if cores_per_die * threads_per_core > 1 {
            CpuidPatch::set_cpuid_reg(cpuid, 0x0000_0001, Some(0), CpuidReg::EBX, ((id as u32) << 24) | (8 << 8) | (((cores_per_die * threads_per_core) as u32) << 16));
            CpuidPatch::set_cpuid_reg(cpuid, 0x0000_0001, Some(0), CpuidReg::EDX, 1 << 28u32);
            CpuidPatch::set_cpuid_reg(cpuid, 0x8000_0008, Some(0), CpuidReg::ECX, /*((thread_width * core_width) << 12) |*/ ((cores_per_die * threads_per_core) - 1) as u32);
        } else {
